...
namespace Admin\Permission\Traits;
 ...
 * - 内部保留 _seq 自增列，仅用于编码输入，不对外暴露。
 * - 不同模型使用基于表名的独立 salt，确保相同序列号产生不同 hash。
 */
trait HasHashId
{
    /**
     * 启动 trait：在 creating 事件中自动生成 hash_id。
     */
    public static function bootHasHashId(): void
    {
        static::creating(function (Model $model) {
            if (empty($model->hash_id)) {
                ...
                $nextSeq        = DB::transaction(function () use ($table) {
                    $result = DB::select(
                        "SELECT COALESCE(MAX(_seq), 0) + 1 as next_seq FROM `{$table}` FOR UPDATE"
                    );
    ...
    {
        $baseSalt = config('permission.hashids.salt', 'admin-permission-rbac');
